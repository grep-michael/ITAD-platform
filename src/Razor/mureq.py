...
def request(method: str, url: str, *, read_limit: int = None,  **kwargs) -> "Response":
...
def post(url: str, body: bytes  = None, **kwargs) -> "Response":
...
def put(url: str, body: bytes  = None, **kwargs) -> "Response":
    """put performs an HTTP PUT request."""
    return request('PUT', url=url, body=body, **kwargs)


def patch(url: str, body: bytes  = None, **kwargs) -> "Response":
...
def _check_redirect(url: str, status: int, response_headers: HTTPMessage) -> str :
...
def _prepare_outgoing_headers(headers: Headers) -> HTTPMessage:
    if headers is None:
        headers = HTTPMessage()
...
def _prepare_body(body, form, json, headers) -> bytes :
    if body is not None:
